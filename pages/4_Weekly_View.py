...
class UI_For_Daily_View:
    ...
    def transformation0(self):
        ...
        # add week column
        self.data['Week'] = self.data['Shift date'].apply(lambda x: x.week if x.week < 53 else 1)
        # add day of the 
        #st.stop()
    ...
    def create_sidebar(self):
        ...
        self.division = st.sidebar.multiselect('Division', self.unique_division, key = 'division_weekly')
        self.restaurant = st.sidebar.multiselect('Restaurant', self.unique_restaurant, default=self.unique_restaurant[0], key = 'restaurant_weekly')
        #self.day_of_the_week = st.sidebar.multiselect('Day of the week', self.unique_days_of_week)
        self.week = st.sidebar.selectbox('Week', self.data['Week'].unique(), key = 'week')
        if self.week is not None:
            self.data = self.data[self.data['Week'] == self.week]
        unique_shift_dates = self.data['Shift date'].unique()
        unique_shift_dates = pd.to_datetime(unique_shift_dates, dayfirst=True)
        unique_shift_dates =  sorted(unique_shift_dates)
        self.unique_shift_dates =unique_shift_dates
    ...
    def create_daily_heatmap(self):
        #self.transformationSalesData()
        # open the data for sales
        ...
        #self.data_for_sales.to_csv('data/data_for_sales.csv', index=True)
        for date in self.unique_shift_dates:
            ...
            with st.expander(f'{date.date()} - {date.day_name()} - Average Time: {average_across_all:.0f} minutes', expanded=False):
                ...
                # preparing the sales data for plotting                    
                # transform date column to datetime
                self.data_for_sales['Date'] = pd.to_datetime(self.data_for_sales['Date'])
                # transform date and column to datetime
                date = date.date()
                data_for_sales = self.data_for_sales
